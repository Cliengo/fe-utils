...
class Analytics {
  ...
  static trackEvent(eventProps = {}) {
    ...
    const traits = this.traits || window.analyticsTraits;
    if (window?.analytics?.track) {
      return window.analytics.track(eventName, { ...traits, eventName, ...props });
    }
  ...
  static trackPage(path, websiteId, url) {
    ...
    if (window?.analytics?.page) {
      ...
      return window.analytics.page(path, {
        ...traits, path, name: path, url: trackUrl, websiteId,
      });
    }
  ...
  static isAnonymousMode() {
    return window.analyticsInAnonymousMode;
  }
